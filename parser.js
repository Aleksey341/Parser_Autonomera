...
class AutonomeraParser {
    constructor(options = {}) {
        ...
        this.concurrentRequests = options.concurrentRequests || 20; // Параллельные запросы (снижено с 500 для совместимости)
        this.requestDelayMs = options.requestDelayMs || 100; // Задержка между параллельными запросами (увеличено с 50 для надежности)
        this.listings = [];
        this.errors = [];
        this.browser = null;
        this.page = null;
    ...
    async fetchListingsChunk(page, startIndex) {
        try {
            ...
            if (startIndex === 0 && newHtml && newHtml.length > 0) {
                console.log(`📥 Первый ответ (${newHtml.length} символов):`);
                console.log(newHtml.substring(0, 500)); // Первые 500 символов
            } else if (startIndex === 0) {
                console.log('⚠️  Первый ответ пустой! Сайт может блокировать запросы или URL неправильный');
                console.log(`🔗 URL: ${url}`);
            }
